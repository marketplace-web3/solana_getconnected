...
export class Solana {
  ...
  async mint(ownerKeyPair: Keypair, token: Token) {
    if (this.connection) {
      const mintAccountToken = await SPLToken.Token.createMint(
        this.connection,
        ownerKeyPair,
      ...
      const creators = [
        new Creator({
          address: ownerKeyPair.publicKey.toBase58(), verified: false, share: 100
        })
      ]
      ...
      await mintAccountToken.setAuthority(
        mintAccountToken.publicKey,
        null,
        "MintTokens",
        ownerKeyPair.publicKey,
        []
